...
class ImageRepository:
    ...
    async def _load_image(self, img_path: str) -> np.ndarray:
    ...
    async def _load_image_with_info(self, img_id, label=None):
    ...
    async def load_training_data(self, csv_path: str, max_images: int = None):
    ...
    async def load_test_data(self, start_id: int = 2701, end_id: int = 3000):
        logger.info(f"Loading test images from {start_id} to {end_id}")
        
        # Create tasks for parallel image loading
        tasks = [
            self._load_image_with_info(img_id)
