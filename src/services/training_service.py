...
class TrainingService:
    ...
    async def train(self, train_csv: str, use_cache: bool = True):
        logger.info("=" * 60)
        logger.info("Facial Expression Recognition System")
        logger.info(f"Cache: {'ON' if use_cache else 'OFF'}")
        logger.info("=" * 60)
        
        feature_extractor = await self.storage.load_feature_extractor() if use_cache else None
        ...
        if feature_extractor is None:
            ...
            features = await feature_extractor.transform(images, context="training images")
